...
# Evaluation Function
def hate_evaluation_google(text_list, labels, max_workers=10):
    print('Generating Google Scores')

    all_scores = [None] * len(text_list)
    all_labels = [None] * len(text_list)

    client = language.LanguageServiceClient(client_options={'api_key': 'AIzaSyCBvGgSxg8ad4Mf-yQxpbA_LJZTqZD8aj4'}) #access key details
