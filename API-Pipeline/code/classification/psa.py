...
# load pre-processed real dataset
real_maj = pd.read_csv(input_path + 'psa_real_data_majority.csv')
real_min = pd.read_csv(input_path + 'psa_real_data_minority.csv')

# load pre-processed synthetic dataset
synth_maj = pd.read_csv(input_path + 'psa_real_data_majority.csv')
synth_min = pd.read_csv(input_path + 'psa_real_data_minority.csv')
...
def api_pred(df, sampl=1, api = 'all'):
    ...
    sampled_df['id'] = sampled_df.psa_id
    if api == 'all':
        results = {
...
def main():
    ...
    if dataset_name == 'real':
        classify_group(real_maj, 'real_maj', sample_frac, api)
        classify_group(real_min, 'real_min', sample_frac, api)
    elif dataset_name == 'synthetic':
        classify_group(synth_maj, 'synthetic_maj', sample_frac, api)
        classify_group(synth_min, 'synthetic_min', sample_frac, api)
    else:  
        logging.exception("Please provide valid dataset name. Choose from real, synthetic")
        return
