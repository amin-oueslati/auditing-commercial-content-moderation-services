...
def build_bert_lstm_model(output_dim, max_seq_length, max_word_length, n_target_groups):
    """
    ...
    # Get BERT embeddings
    bert_embeddings = bert_model(input_ids)[0]  # We use the first output (last hidden state)

    # LSTM layer
    lstm_layer = Bidirectional(LSTM(units=output_dim, recurrent_dropout=0.2, name="LSTM_Layer"), name="LSTM_Bi_Layer")(bert_embeddings)

    # Output layer
    output = Dense(n_target_groups, activation="softmax", name="Output")(lstm_layer)
...
def build_model(input_dim, output_dim, max_seq_length, max_word_length, n_target_groups, embedding_matrix):
    ...
    """
   #  Word-level input
    word_input = Input(shape=(max_seq_length,), name="Input_Layer")
    #word_embedding = Embedding(input_dim=input_dim,output_dim = output_dim, weights = [embedding_matrix])(word_input)

    word_embedding = Embedding(input_dim=input_dim, output_dim=output_dim, trainable=False, name="Embedding_Layer")(word_input) # D: I'll set the weight further down. Also I added trainable=False, because we're using pre-trained embeddings

    word_lstm = Bidirectional(LSTM(units=output_dim, recurrent_dropout=0.2, name="LSTM_Layer"), name="LSTM_Bi_Layer")(word_embedding)
    ...
    output = Dense(n_target_groups, activation="softmax", name="Output_Layer")(word_lstm)
    # Define model
...
def get_pad_train_test_val(data, max_seq_length, max_word_length):
    # Assuming you have 'text', 'Sent_idx', and 'Label_idx' columns in your DataFrame 'data'

    # Tokenize sentences and words, and get maximum token and tag length
    X_word,_,_ = tokenize(data.text, data.words, max_seq_length, max_word_length)
...
def main():
    ...
    if os.path.exists(model_file_path):
        ...
        model = build_bert_lstm_model(32, 512, 100, 10)
